    ...
    public class Admin : Controller
    {
        ...
        public async Task<IActionResult> Reject([FromBody] VerificationActionRequest request)
        {
            ...
            try
            {
                ...
                var ok = await UpdateAndNotifyRejectAsync(request.Id, reason);
                if (!ok)
        ...
        // helper to update status and create notification for rejection
        private async Task<bool> UpdateAndNotifyRejectAsync(Guid verificationId, string reason)
        {
            var updated = await _verificationService.UpdateVerificationStatusAsync(verificationId, "REJECTED", reason);
            if (!updated) return false;

            var verification = await _context.IdentityVerifications.FirstOrDefaultAsync(v => v.Id == verificationId);
            ...
            await _notificationService.CreateNotificationAsync(
                ...
                $"Your identity verification was declined. Reason: {reason}",
                "identity_declined",
