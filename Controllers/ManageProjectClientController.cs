    ...
    public class ManageProjectClientController : Controller
    {
        ...
        public async Task<IActionResult> SetDeadline(Guid projectId, DateTime deadline)
        {
            ...
            try
            {
                ...
                {
                    // Assume the deadline input is in Philippine Time and convert to UTC
                    var phTimeZone = TimeZoneInfo.FindSystemTimeZoneById("Asia/Manila");
                    utcDeadline = TimeZoneInfo.ConvertTimeToUtc(deadline, phTimeZone);
                }
                ...
                {
                    utcDeadline = deadline; // Already UTC
                }

                // Validate deadline is in the future (compare with UTC)
                ...
                // Update the project deadline with UTC time
                project.Deadline = utcDeadline;
                await dbContext.SaveChangesAsync();

                // Use the NotificationService - convert back to PHT for display in notification
                var phDisplayTime = TimeHelpers.ToPhilippineTime(utcDeadline);
                await notificationService.CreateNotificationAsync(
                    project.AcceptedBid.UserId,
                    "New Project Deadline Set",
                    $"A deadline has been set for project '{project.ProjectName}' - Due: {phDisplayTime:MMM dd, yyyy 'at' h:mm tt}",
                    "Deadline",
            ...
            {
                TempData["ErrorMessage"] = $"Database error occurred while setting the deadline: {dbEx.InnerException?.Message ?? dbEx.Message}";
                return RedirectToAction(nameof(Details), new { id = projectId });
            }
            catch (Exception ex)
            {
                TempData["ErrorMessage"] = $"An error occurred while setting the deadline: {ex.Message}";
                return RedirectToAction(nameof(Details), new { id = projectId });
            }
        }
    }
