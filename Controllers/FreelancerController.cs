...
namespace Freelancing.Controllers
{
    ...
    public class FreelancerController : Controller
    {
        ...
        public FreelancerController(ApplicationDbContext context, INotificationService notificationService, UserManager<UserAccount> userManager, SignInManager<UserAccount> signInManager, IEmailService emailService, IGoogleCloudStorageService googleCloudStorageService)
        {
            ...
            this._googleCloudStorageService = googleCloudStorageService;
        }
        ...
        public async Task<IActionResult> Bid(ViewProjectandBidding viewModel)
        {
            ...
            // Handle file uploads for previous works using Google Cloud Storage
                ...
                foreach (var file in viewModel.Bidding.PreviousWorksFiles)
                {
                    if (file.Length > 0)
                    {
                        ...
                        try
                        {
                            ...
                            var publicUrl = await _googleCloudStorageService.UploadFileAsync(file, "previous-works");
                            uploadedFilePaths.Add(publicUrl);
                        }
                        catch (Exception ex)
                        {
                            ModelState.AddModelError("PreviousWorksFiles", $"Failed to upload {file.FileName}: {ex.Message}");
                            viewModel.Project = project;
                            return View(viewModel);
                        }
                    }
        ...
        public async Task<IActionResult> EditBid(Guid id, ViewProjectandBidding viewModel, string action, string removedFiles)
        {
            ...
            if (action == "save")
            {
                ...
                // Handle file removal from Google Cloud Storage
                ...
                // Remove files from existing paths and delete from Google Cloud Storage
                foreach (var filePath in filesToRemove)
                {
                    if (existingFilePaths.Contains(filePath))
                    {
                        ...
                        try
                        {
                            ...
                            await _googleCloudStorageService.DeleteFileAsync(filePath);
                        }
                        catch (Exception ex)
                        {
                            // Log the error but don't fail the operation
                            Console.WriteLine($"Error deleting file from cloud storage {filePath}: {ex.Message}");
                        }
                    }
                }
            ...
            {
                // Delete all associated files from Google Cloud Storage before removing the bidding
                if (!string.IsNullOrEmpty(bidding.PreviousWorksPaths))
                {
                    try
                    {
        ...
        public async Task<IActionResult> EditAccount(EditAccount viewModel, IFormFile? PhotoFile)
        {
            ...
            if (PhotoFile != null && PhotoFile.Length > 0)
            {
                ...
                try
                {
                    // Delete old photo from Google Cloud Storage if exists
                    if (!string.IsNullOrEmpty(userAccount.Photo))
                    {
                        await _googleCloudStorageService.DeleteFileAsync(userAccount.Photo);
                    }

                    // Upload new photo to Google Cloud Storage
                    var publicUrl = await _googleCloudStorageService.UploadFileAsync(PhotoFile, "profiles");
            ...
            // Update email and username logic remains the same...
                ...
                if (portfolioItem.Files != null && portfolioItem.Files.Any())
                {
                    ...
                    foreach (var file in portfolioItem.Files)
                    {
                        if (file.Length > 0)
                        {
                            try
                            {
                                var publicUrl = await _googleCloudStorageService.UploadFileAsync(file, "portfolios");
                                filePaths.Add(publicUrl);
                            }
                            catch (Exception ex)
                            {
                                return Json(new { success = false, message = $"Failed to upload {file.FileName}: {ex.Message}" });
                            }
                        }
                    }
