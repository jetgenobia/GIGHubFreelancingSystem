...
namespace Freelancing.Controllers
{
    ...
    public class PeerMentorshipController : Controller
    {
        ...
        public PeerMentorshipController(ApplicationDbContext context, INotificationService notificationService, UserManager<UserAccount> userManager, SignInManager<UserAccount> signInManager)
        {
            _context = context;
            ...
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    ...
                    if (existingUser != null)
                    {
                        ...
                        existingUser.Role = model.Role; // Update user role too
                        _context.UserAccounts.Update(existingUser);
                        await _context.SaveChangesAsync();

                        // Refresh the user's claims to include all updated information
                        // Make sure this happens after the database save
                        await RefreshUserClaims(existingUser);
                    }
                    else
                    {
                        // If no existing user found, you might want to handle this case
                        // depending on your application logic
                        throw new InvalidOperationException("User account not found for mentorship registration");
                    }
